...
void	norm_f_rdr(char **envp, t_commands *commands, t_rdr *t_cmd, int *i)
{
	...
	else if (commands->command[*i] == '>' || commands->command[*i] == '<')
	{
		...
		commands->filerdr->name = get_rdr_file(envp, commands->command, *i);
...
int	files_rdr(char **envp, t_commands *commands)
{
	...
	while (++i < (int)ft_strlen(commands->command))
		norm_f_rdr(envp, commands, t_cmd, &i);
